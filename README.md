...
Packet-NPC плагин для **Paper 1.21**: player- и mob-модели через **PacketEvents**, голограммы на **TextDisplay / ItemDisplay**, GUI-редактор, гибкие клики, async-хранилище и public API для других плагинов.
...
JDBC-библиотеки (HikariCP, SQLite, MySQL) подтягиваются через `plugin.yml` → `libraries` — в JAR не вшиваются.
...
4. Создать NPC: `/soulnpc create` или `/soulnpc edit`.

---

## Архитектура

- **Отображение:** fake-сущности через PacketEvents; на main thread только cache и игровая логика.
- **Хранение:** in-memory cache (`CachedNpcRepository`); запись/чтение с диска или БД — **async**, результат применяется через scheduler на main thread.
- **Конфиг:** [Elytrium Serializer](https://github.com/Elytrium/java-serializer) — дефолты в Java-классах, на диске merge при reload.
- **GUI:** custom `InventoryHolder` на каждое меню; проверка прав на открытие и на каждый клик.

```
Main thread:  NpcService / Spawn / Interaction / GUI  ↔  cache
Async:        YamlStorageBackend | SqliteStorageBackend | MySqlStorageBackend
```
...
|------------|---------|------------|
| `soulnpc.use` | **true** | Клики по NPC (игроки) |
...
`soulnpc.admin` включает create / delete / edit. Обычный игрок с дефолтными правами видит только NPC и может с ними взаимодействовать (`use`).

Опасные действия в GUI дополнительно проверяются: reload / tp / respawn — **admin**, удаление — **delete**.
...
JAR: `build/libs/SoulNPC-*.jar`.

---

## Зависимости (libraries в plugin.yml)

- `net.elytrium:serializer:1.1.1`
